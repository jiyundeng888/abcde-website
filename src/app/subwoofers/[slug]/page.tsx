...
export function generateStaticParams() {
  return Object.keys(productDetails)
    .filter((slug) => productDetails[slug].categorySlug === "subwoofers")
    .map((slug) => ({ slug }));
}
...
export default async function SubwooferProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!product || product.categorySlug !== "subwoofers") {
    notFound();
  }
  const relatedProducts = getRelatedProducts("subwoofers", slug);
  return (
    <SiteLayout>
