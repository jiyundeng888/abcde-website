...
export function generateStaticParams() {
  return getProductSlugsForCategory("sleds").map((slug) => ({ slug }));
}

export function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  return params.then(async (resolvedParams) => {
    const product = await getProductDetailData(resolvedParams.slug, "sleds");
    return {
...
export default async function SledProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const product = await getProductDetailData(slug, "sleds");
  if (!product) {
    notFound();
  }
  const relatedProducts = await getRelatedProductsFromPrismic("sleds", slug);
  return (
    <SiteLayout>
